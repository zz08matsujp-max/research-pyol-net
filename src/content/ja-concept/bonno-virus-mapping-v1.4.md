...
summary: "私の研究プログラムの最深層の目的は、人間を煩悩の構造として理解し、その動的な重み配分を計算可能な座標系で可視化することにある。仏教 2,500 年の内省的観察を、現代ニューラルネットワーク工学の座標系として採用する。詐欺対策（SNS型国際投資詐欺・殺豬盤）は、その最も差し迫った応用例である。v1.6 で §0、v1.7 で §16.5、v1.8 で §0.4.5 / §4.3.0、v1.9 で文体調整、v2.0 で整合性 11 点、v2.1 で §17 ExPFC Core Specification v0.1 を新設し、研究成果物を『アプリ』ではなく『共通語彙（仕様 + オントロジー + 参照実装）』として位置付ける戦略を確定。"
...
ここで wᵢ は **「テキスト T が、状況 C のもとで、個人 U の煩悩ᵢ をどれだけ刺激するか」** の強度を表す。値域は [0, 1] に正規化する。§0.4 で示した「人間の状態 (T, C, U) → V」と完全に同じ写像である。
...
アプリ実装が Core 仕様にどこまで準拠するかを、3 段階で定義する：

| Level | 出力次元 | 必須機能 | 想定用途 |
|---|---|---|---|
| **L1（MVP）** | 9 次元（三毒 × 3） | f, detect, judge | 初期実装、開発者検証、PoC |
| **L2（実用）** | 10〜20 次元 | L1 + 出典明示性, U プロファイル完全 | Phase 1 本番アプリ |
| **L3（完全形）** | 108 次元 | L2 + 階層的埋め込み, episodic 学習 | Phase 2 以降、研究用途 |

実装は「ExPFC Core L1 準拠」「L2 準拠」を明示する。これにより、ユーザーはアプリの能力レベルを一目で把握できる。
...
- v2.2 以降: 実装進捗、外部協力者との対話、検証結果に応じて更新
