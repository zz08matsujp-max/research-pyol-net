...
summary: "私の研究プログラムの最深層の目的は、人間を煩悩の構造として理解し、その動的な重み配分を計算可能な座標系で可視化することにある。仏教 2,500 年の内省的観察を、現代ニューラルネットワーク工学の座標系として採用する。詐欺対策（SNS型国際投資詐欺・殺豬盤）は、その最も差し迫った応用例である。v1.6 で §0『研究の根本目的』を新設、v1.7 で §16.5『個人化問題と煩悩感受性プロファイル』を追補、v1.8 で §0.4.5『3×3 ミニマムモデル』を追補、v1.9 で文体を『私』基調へ調整、v2.0 で整合性 11 点を一括修正（写像式の統一・References 補完・コスト試算更新・§16 番号修正など）。"
...
- v2.1 以降: 実装進捗、外部協力者との対話、検証結果に応じて更新
